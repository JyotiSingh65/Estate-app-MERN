...
export const google = async (req, res, next) => {
    try {
        ...
        if(user){
            ...
            const { password: userPassword, ...rest } = user._doc; // get the user info without the password
            res.cookie("access_token", token, { httpOnly: true })
            .status(200)
            .json(rest); // send the token in a cookie
            ...
            const { password: userPassword, ...rest } = newUser._doc; // get the user info without the password
            res.cookie("access_token", token, { httpOnly: true })
            .status(200)
            .json(rest) // send the token in a cookie
